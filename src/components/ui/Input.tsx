...
const base = 'w-full rounded-2xl border border-[rgb(var(--border))]/90 bg-white/70 px-4 py-3 text-sm text-[rgb(var(--foreground))] shadow-innerGlow outline-none backdrop-blur transition placeholder:text-[rgb(var(--muted-foreground))]/65 focus:border-turquoise focus:ring-4 focus:ring-turquoise/15 dark:border-[rgb(var(--border))] dark:bg-[rgb(var(--card))] dark:text-[rgb(var(--foreground))]';
...
export function Select({ className, ...props }: SelectHTMLAttributes<HTMLSelectElement>) { return <select className={cn(base, 'appearance-none', className)} {...props} />; }
